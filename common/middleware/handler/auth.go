...
func Authenticator(c *gin.Context) (interface{}, error) {
	...
	if mongoDB, ok := db.(*mongo.Database); ok {
		sysUser, role, e = loginVals.GetUserWithMongo(mongoDB)
	} else if gormDB, ok := db.(*gorm.DB); ok {
		sysUser, role, e = loginVals.GetUser(gormDB)
	}
...
func LogOut(c *gin.Context) {
	...
	db, err := pkg.GetMongo(c)
	if err != nil {
		pkg.HasError(err, "数据库连接获取失败", 500)
	}
	// jwt加入黑名单
	jwtService := &jwt.JwtService{}

	err = jwtService.JsonInBlacklist(token, db)
	if err != nil {
		pkg.HasError(err, "jwt作废失败", 500)
	}
	jwt := models2.JwtBlacklist{Jwt: token}
