...
type Runtime interface {
	...
	SetMongoClient(key string, db *mongo.Client)
	GetMongoClient() map[string]*mongo.Client
	GetMongoClientByKey(key string) *mongo.Client
	SetMongoDb(key string, db *mongo.Database)
	GetMongoDb() map[string]*mongo.Database
	SetDb(key string, db *gorm.DB)
	GetDb() map[string]*gorm.DB
	GetDbByKey(key string) *gorm.DB
	GetMongoDbByKey(key string) *mongo.Database
