...
type Application struct {
	...
	mongoClients map[string]*mongo.Client // 新增的 MongoDB 客户端映射
...
// SetMongoClient 设置 MongoDB 数据库连接
func (e *Application) SetMongoClient(key string, client *mongo.Client) {
	e.mux.Lock()
	defer e.mux.Unlock()
	e.mongoClients[key] = client
}

// GetMongoDb 获取所有 MongoDB 数据库连接
func (e *Application) GetMongoClient() map[string]*mongo.Client {
	e.mux.Lock()
	defer e.mux.Unlock()
	return e.mongoClients
}

func (e *Application) GetMongoClientByKey(key string) *mongo.Client {
	e.mux.Lock()
	defer e.mux.Unlock()
	if client, ok := e.mongoClients["*"]; ok {
		return client
	}
	return e.mongoClients[key]
}

func (e *Application) SetMongoDb(key string, db *mongo.Database) {
	e.mux.Lock()
	defer e.mux.Unlock()
	e.mongoDbs[key] = db
}

func (e *Application) GetMongoDb() map[string]*mongo.Database {
	e.mux.Lock()
	defer e.mux.Unlock()
	return e.mongoDbs
}
...
func (e *Application) GetMongoDbByKey(key string) *mongo.Database {
	e.mux.Lock()
	defer e.mux.Unlock()
	if db, ok := e.mongoDbs["*"]; ok {
		return db
	}
	return e.mongoDbs[key]
}
