...
func (e *Api) MakeMongo() *Api {
	var err error
	if e.Logger == nil {
		err = errors.New("at MakeMongo logger is nil")
		e.AddError(err)
		return e
	}
	db, err := pkg.GetMongo(e.Context)
	if err != nil {
		e.Logger.Error(http.StatusInternalServerError, err, "数据库连接获取失败")
		e.AddError(err)
	}
	e.Mongo = db
	// db.mongo可以继续链式调用Client
	//client, err := pkg.GetMongoClient(e.Context)
	//if err != nil {
	//	e.Logger.Error(http.StatusInternalServerError, err, "数据库连接获取失败")
	//	e.AddError(err)
	//}
...
func (e *Api) MakeOrmOrMongo() *Api {
	databaseConfig := sdkConfig.DatabaseConfig
	var err error
	if e.Logger == nil {
		err = errors.New("at MakeOrmOrMongo logger is nil")
		e.AddError(err)
		return e
	}
	if databaseConfig.Driver == "mongo" {
		db, err := pkg.GetMongo(e.Context) // 使用 GetMongo
		if err != nil {
			e.Logger.Error(http.StatusInternalServerError, err, "数据库连接获取失败")
			e.AddError(err)
		}
		...
		db, err := pkg.GetOrm(e.Context) // 使用 GetOrm
		if err != nil {
			e.Logger.Error(http.StatusInternalServerError, err, "数据库连接获取失败")
			e.AddError(err)
		}
