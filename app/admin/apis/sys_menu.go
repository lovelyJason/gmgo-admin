...
// @Success 200 {object} response.Response "{"code": 200, "data": [...]}"
// @Router /api/v1/menu/edit/{id} [post]
// @Security Bearer
...
// @Security Bearer
// 好像目前前端传递的roleId是0，好像跟角色无关啊
func (e SysMenu) GetMenuTreeSelect(c *gin.Context) {
	menuService := service.SysMenu{}
	roleService := service.SysRole{}
	req := dto.SelectRole{}
	err := e.MakeContext(c).
		MakeMongo().
		MakeService(&menuService.Service).
	...
	result, err := menuService.SetLabel()
	if err != nil {
		e.Error(500, err, "查询失败")
		return
	}
	...
	menuIds := make([]int, 0)
	if req.RoleId != 0 {
		menuIds, err = roleService.GetRoleMenuId(req.RoleId)
		if err != nil {
			e.Error(500, err, "")
			return
		}
