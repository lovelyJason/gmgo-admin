...
func (e *SysDept) getList(c *dto.SysDeptGetPageReq, list *[]models.SysDept) error {
	...
	filter := models.SysDept{}
	err = model.List(e.Mongo, filter, 0, 0, list)
...
// GetDeptPage 设置dept页面数据
func (e *SysDept) GetDeptPage(c *dto.SysDeptGetPageReq) (m []models.SysDept, err error) {
	var list []models.SysDept
	err = e.getList(c, &list)
	for i := 0; i < len(list); i++ {
		if list[i].ParentId != 0 {
