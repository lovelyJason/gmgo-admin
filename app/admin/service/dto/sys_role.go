...
type SysRoleInsertReq struct {
	RoleId    int                  `uri:"id" form:"roleId" comment:"角色编码"` // 角色编码
	...
	Menus     []primitive.ObjectID `form:"menus"`
...
func (s *SysRoleInsertReq) GenerateBson() bson.M {
	filter := bson.M{}
	filter["roleName"] = s.RoleName
	filter["status"] = s.Status
	filter["roleKey"] = s.RoleKey
	filter["roleSort"] = s.RoleSort
	filter["flag"] = s.Flag
...
func (s *SysRoleUpdateReq) GenerateBson() bson.M {
	filter := bson.M{}
	filter["roleName"] = s.RoleName
	filter["status"] = s.Status
	filter["roleKey"] = s.RoleKey
