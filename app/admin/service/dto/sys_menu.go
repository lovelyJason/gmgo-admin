...
type SysMenuInsertReq struct {
	...
	Apis             []primitive.ObjectID `form:"apis"`
	...
	common.ControlBy `bson:"inline"`
	common.ModelTime `bson:"inline"`
}
...
func (s *SysMenuInsertReq) GenerateBson() bson.M {
	filter := bson.M{}
	filter["menuName"] = s.MenuName
	filter["title"] = s.Title
	filter["icon"] = s.Icon
	filter["path"] = s.Path
	filter["paths"] = s.Paths
	filter["menuType"] = s.MenuType
	filter["action"] = s.Action
	filter["action"] = s.Action
	filter["apis"] = s.Apis
	filter["permission"] = s.Permission
	filter["parentId"] = s.ParentId
	filter["noCache"] = s.NoCache
	filter["breadcrumb"] = s.Breadcrumb
	filter["component"] = s.Component
	filter["sort"] = s.Sort
	filter["visible"] = s.Visible
	filter["isFrame"] = s.IsFrame
	if s.CreateBy != 0 {
...
func (s *SysMenuUpdateReq) GenerateBson() bson.M {
	filter := bson.M{}
	filter["menuName"] = s.MenuName
	filter["title"] = s.Title
	filter["icon"] = s.Icon
...
type SysMenuGetReq struct {
	Id int `uri:"id"` // 注意前端传递过来的实际是menuId
}
...
type MenuLabel struct {
	Id       primitive.ObjectID `bson:"-" json:"id,omitempty"`
	MenuId   int                `bson:"-" json:"menuId,omitempty"`
	Label    string             `bson:"-" json:"label,omitempty"`
	Children []MenuLabel        `bson:"-" json:"children,omitempty"`
}
